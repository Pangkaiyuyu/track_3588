/************************************************************************
 *FileName:  Midware_MPP.cpp
 *Author:    Jin Qiuyu
 *Version:   2.0
 *Date:      2020-03-08
 *Description: 中间件-MPP方法
 ************************************************************************/
...
MPP_RET test_mpp_run(MpiEncMultiCtxInfo *info)
{
	...
	while (!p->pkt_eos) 
	{
		...
		p_temp_1080 = queue_img_src_to_vpss.ReadDat();
		...
		cv::Mat frame_show(720, 1280, CV_8UC3, p_temp_1080->p_buf);//Gray
		rectangle(frame_show, Rect(roi_1,roi_2,roi_3,roi_4), Scalar(0, 255, 255), 2 , 8, 0);//用矩形画矩形窗
		memcpy(buf, frame_show.data, p_temp_1080->n_length);

		// memcpy(buf, p_temp_1080->p_buf, p_temp_1080->n_length);

		//printf("p_temp_1080->n_length = %d\n",p_temp_1080->n_length);
		...
		do 
		{
			...
			if (packet) 
			{
				...
				stream_pack *p_temp_postback = new stream_pack();
				memcpy(p_temp_postback->p_buf, ptr, len);
				p_temp_postback->n_length = len;
				queue_stream_to_out.WriteDat(p_temp_postback);
...
int test_mpp(int argc, char **argv)
{
	RK_S32 ret = MPP_NOK;
...
MpiEncTestArgs *mpi_enc_num_set(int venc_num)
{
	MpiEncTestArgs *args = mpp_calloc(MpiEncTestArgs, venc_num);

	if (args)
	{
		args->nthreads = venc_num;
		return args;
	}
	else
	{
...
void show_enc_fps(RK_S64 total_time, RK_S64 total_count, RK_S64 last_time, RK_S64 last_count)
{
	...
	printf("encoded %10lld frame fps avg %7.2f ins %7.2f\n",total_count, avg_fps, ins_fps);
}

MPP_RET mpi_enc_check(MpiEncTestArgs* cmd)
{
	MPP_RET ret = MPP_OK;
...
int enc_start(MpiEncTestArgs* cmd)
{
	RK_S32 ret = MPP_OK;
	MpiEncMultiCtxInfo *ctxs = NULL;
...
int enc_stop(MpiEncTestArgs* cmd)
{
...
{
	RK_S32 ret = MPP_NOK;
	MpiEncTestArgs* cmd = mpi_enc_num_set(1);

	if (cmd == NULL)
	{
		printf("MpiEncTestArgs init failed !\n");
		return -1;
	}
	...
	ret = mpi_enc_check(cmd);

	if (ret == MPP_NOK)
	{
		printf("mpi_enc_check failed !\n");
		return -1;
	}
